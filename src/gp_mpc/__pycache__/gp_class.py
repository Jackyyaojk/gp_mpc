...
from .gp_functions import build_gp, build_TA_cov, get_mean_function, build_gp_fast
...
class GP:
    def __init__(self, X, Y, mean_func="zero", gp_method="TA",
        ...
        self.__Nx = X.shape[1]
        self.__N = X.shape[0]
        self.__Nu = 0#self.__Nx - self.__Ny
        ...
        if meta is not None:
            self.__meanY = np.array(meta['meanY'],dtype=self.dtype)
            self.__stdY  = np.array(meta['stdY'],dtype=self.dtype)
            self.__meanZ= np.array(meta['meanZ'],dtype=self.dtype)
            self.__stdZ = np.array(meta['stdZ'],dtype=self.dtype)
            self.__meanX = np.array(meta['meanX'],dtype=self.dtype)
            self.__stdX = np.array(meta['stdX'],dtype=self.dtype)
            self.__meanU = np.array(meta['meanU'],dtype=self.dtype)
            self.__stdU  = np.array(meta['stdU'],dtype=self.dtype)
        ...
        if hyper is None:
            ...
            self.__meanY = np.mean(Y, 0,dtype=self.dtype)
            self.__stdY  = np.std(Y, 0,dtype=self.dtype)
            self.__meanZ = np.mean(X, 0,dtype=self.dtype)
            self.__stdZ  = np.std(X, 0,dtype=self.dtype)
            self.__meanX = np.mean(X[:, :self.__Nx], 0,dtype=self.dtype)
            self.__stdX  = np.std(X[:, :self.__Nx], 0,dtype=self.dtype)
            self.__meanU = np.mean(X[:, self.__Nx:], 0,dtype=self.dtype)
            self.__stdU  = np.std(X[:, self.__Nx:], 0,dtype=self.dtype)
            self.__hyper_length_scales   = np.array(hyper['length_scale'],dtype=self.dtype)
            self.__hyper_signal_variance = np.array(hyper['signal_var'],dtype=self.dtype)
            self.__hyper_noise_variance  = np.array(hyper['noise_var'],dtype=self.dtype)
            self.__hyper_mean = np.array(hyper['mean'],dtype=self.dtype)
            self.set_hyper_parameters_limited(hyper)
            self.replace_data_all(X, Y)
    ...
    def optimize(self, X=None, Y=None, opts=None, mean_func='zero',
        ...
        if normalize and X is not None:
            self.__xlb = np.array(xlb,dtype=self.dtype)
            self.__xub = np.array(xub,dtype=self.dtype)
            self.__ulb = np.array(ulb,dtype=self.dtype)
            self.__uub = np.array(uub,dtype=self.dtype)
            self.__lb = np.hstack([xlb, ulb],dtype=self.dtype)
            self.__ub = np.hstack([xub, uub],dtype=self.dtype)
            self.__meanY = np.mean(Y, 0,dtype=self.dtype)
            self.__stdY  = np.std(Y, 0,dtype=self.dtype)
            self.__meanZ = np.mean(X, 0,dtype=self.dtype)
            self.__stdZ  = np.std(X, 0,dtype=self.dtype)
    ...
    def predict(self, x, u, cov):
        ...
        #if np.any(cov<0.0): print("Covariance has negative components, setting to min")
        if self.__normalize:
            mean = self.inverse_mean(mean, self.__meanY, self.__stdY)
            cov = self.inverse_variance(cov)
        return mean, cov

    def predict_lik(self, x, y, lik_floor = None, print_lik = False):
        """ Give log likelihod of a model having that observation
        """
        if self.__normalize:
            x_s = self.standardize(x, self.__meanX, self.__stdX)
            y_s = self.standardize(y, self.__meanY, self.__stdY)
        else:
            x_s = x
            y_s = y
        mean = self.__mean(x_s)
        cov  = self.__covar(x_s)
        if np.any(cov<0.0): print("GP covariance has negative components")
        err  = y_s-mean
        lik = []
        for i in range(len(y)): # covariance is diagonal, so we decompose the total likelihood
            if not np.isnan(cov[i,i]) and not np.isnan(err[i]):
               lik.append( -0.5*np.log(cov[i,i])-err[i]**2/(cov[i,i]+0.5)) # the plus 0.2 helps the GP be a bit less over-confident
               #lik.append( -(err[i]/cov[i,i])**2)
        lik = np.array(lik)
        if lik_floor: lik[np.where(lik<lik_floor)] = lik_floor
        if print_lik: print(lik)
        return np.sum(lik)
    ...
    def get_xrange(self):
        if self.__normalize: 
            minx = self.inverse_mean(np.min(self.__X, axis=0), self.__meanX, self.__stdX)
            maxx = self.inverse_mean(np.max(self.__X, axis=0), self.__meanX, self.__stdX)
        else:
    ...
    def set_hyper_parmaeters(self, hyper):
        """ Set hyper-parameters
        """
        #self.__hyper_length_scales = hyper['length_scale']
        self.__hyper  = np.array(hyper['hyper'])
        self.__invK   = np.array(hyper['invK'], dtype = self.dtype)
        self.__alpha  = np.array(hyper['alpha'], dtype = self.dtype)
        self.__chol   = np.array(hyper['chol'], dtype = self.dtype)
    ...
    def set_hyper_parameters_limited(self, hyper):
        self.__hyper_length_scales   = hyper['length_scale']
        self.__hyper_signal_variance = hyper['signal_var']
        self.__hyper_noise_variance  = hyper['noise_var']
        self.__hyper_mean = np.array(hyper['mean'])
        self.__hyper = np.hstack((hyper['length_scale'], hyper['signal_var'], hyper['noise_var']))
    ...
    def print_hyper_parameters(self):
        ...
        for state in range(self.__Ny):
            ...
            for i in range(self.__Nx + self.__Nu):
    ...
    def covar(self, X_new):
        ...
        for output in range(self.__Ny):
            ...
            covar[output] = kss - ca.mtimes(v.T, v)
        return covar
    ...
    def update_data(self, X_new, Y_new, N_new=None):
        ...
        X_new = X_new.astype(np.single)#np.array(X_new).copy()
        Y_new = Y_new.astype(np.single)#np.array(Y_new).copy()
        n, D = X_new.shape
        if N_new is None:
            N_new = n
        ...
        for k in range(N_new):
            ...
            N, D = self.__X.shape
            hyper = self.__hyper
            invK  = np.zeros((self.__Ny, N + 1, N + 1), dtype = self.dtype )
            alpha = np.zeros((self.__Ny, N + 1), dtype = self.dtype)
            chol  = np.zeros((self.__Ny, N + 1, N + 1), dtype = self.dtype)
            ...
            for output in range(self.__Ny):
                ...
                L = self.__chol[output]
                l_new = np.linalg.solve(L, K_new).astype(np.single)
                l_new__ = np.sqrt(k_new__ - np.linalg.norm(l_new)).astype(np.single)
                chol[output, N:, :N] = l_new.T
                chol[output, N, N] = l_new__
                invL = np.linalg.solve(chol[output], np.eye(N + 1))
                invK[output, :, :] = np.linalg.solve(chol[output].T, invL).astype(np.single)
            ...
            for output in range(self.__Ny):
                ...
                alpha[output] = np.linalg.solve(chol[output].T,
                                     ...
                                     self.__Y[:, output] - mean)).astype(np.single)
            self.__alpha = alpha
            self.__chol = chol
            self.__invK = invK
    ...
    def update_data_all(self, X_new, Y_new):
        ...
        for output in range(self.__Ny):
            ...
            alpha[output] = np.linalg.solve(L.T,
                                            np.linalg.solve(L, self.__Y[:, output] - mean), dtype = self.dtype)
    ...
    def replace_data_all(self, X_new, Y_new):
        ...
        X_new = X_new.astype(self.dtype)#np.array(X_new).copy()
        Y_new = Y_new.astype(self.dtype)#np.array(Y_new).copy()
        n, D = X_new.shape
        ...
        # Rebuild GP with the new data
        self.__mean, self.__var, self.__covar, self.__mean_jac = \
                            build_gp_fast(self.__invK, self.__X, self.__hyper,
                                     self.__alpha, self.__chol)
    ...
    def predict_compare(self, x0, u, model, num_cols=2, xnames=None,
        ...
        for i in range(len(methods)):
            ...
            for t in range(1, Nt + 1):
                ...
                if feedback:
                    covar_u = ca.mtimes(ca.mtimes(K, covar_x), K.T)
                    cov_xu = ca.mtimes(covar_x, K.T)
                    covar[Ny:, Ny:] = covar_u
