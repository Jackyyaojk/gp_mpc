...
def build_gp(invK, X, hyper, alpha, chol, meanFunc='zero'):
    ...
    var_i_func  = ca.Function('var', [v_s, kss_s, sn2_s],
                            [sn2_s + kss_s - ca.mtimes(v_s.T, v_s)])

    for output in range(Ny):
        ell      = ca.SX(hyper[output, 0:Nx])
        sf2      = ca.SX(hyper[output, Nx]**2)
        sn2      = ca.SX(hyper[output, Nx+1]**2)
        ...
        var[output]  = var_i_func(v, sf2, sn2)
...
def build_gp_fast(invK, X, hyper, alpha, chol, meanFunc='zero'):
    ...
    for output in range(Ny):
        ...
        sn2      = hyper[output, Nx+1]**2
        alpha_a  = alpha[output]
        
        covSE =  covSEard_fast(ell, sf2)
        ks_func = covSE.map(N, 'openmp') #can give 'thread' arguments too
        ks       = ks_func(X.T, z_s)
        v        = ca.solve(chol[output], ks.T)
        mean[output] = ks@alpha_a
        var[output]  = sn2 + sf2 - ca.mtimes(v.T, v)

    mean_func  = ca.Function('mean', [z_s], [mean])
    var_func = ca.Function('var', [z_s], [var])
    covar_func = ca.Function('covar',  [z_s], [ca.diag(var)])
    mean_jac_z = ca.Function('mean_jac_z', [z_s],
                                      [ca.jacobian(mean_func(z_s), z_s)])
    return mean_func.expand(), var_func.expand(), covar_func.expand(), mean_jac_z.expand()
