...
#            covar_s = ca.blockcat(covar_x_s, cov_xu, cov_xu.T, cov_u)
    ...
    #            covar_s = ca.blockcat(covar_x_s, cov_xu, cov_xu.T, cov_u)
            ...
            covar_u_func = ca.Function('cov_u', [covar_x_sx, K_sx],
                                       [ca.mtimes(ca.mtimes(K_sx, covar_x_sx), K_sx.T)])
            cov_xu_func = ca.Function('cov_xu', [covar_x_sx, K_sx],
                                      [ca.mtimes(covar_x_sx, K_sx.T)])
            cov_xu = cov_xu_func(covar_x_s, K_s.reshape((Nu, Ny)))
            cov_u = covar_u_func(covar_x_s, K_s.reshape((Nu, Ny)))
            covar_s = ca.blockcat(covar_x_s, cov_xu, cov_xu.T, cov_u)
    ...
    #                                            [Bd @ covar_d_s + jac @ covar_x_s])
        ...
        L_to_cov_func = ca.Function('cov', [L_s], [ca.mtimes(L_s, L_s.T)])
        covar_x_sx = ca.SX.sym('cov_x', Ny, Ny)
...
#            con_ineq_ub.extend(cons['con_ub'])
            ...
            obj += self.__l_func(var['mean', t], covar_x_t, u_t, cov_u, u_delta) \
                    + ca.mtimes(np.full((1, num_slack),lam), var['eps', t])
            if lam_state is not None:
                obj += ca.mtimes(np.full((1,num_state_slack),lam_state), var['eps_state', t])
            u_t = u_past
        L_x = ca.MX(ca.Sparsity.lower(Ny), var['L', Nt])
        covar_x_t = L_to_cov_func(L_x)
        obj += self.__lf_func(var['mean', Nt], covar_x_t, P_s.reshape((Ny, Ny))) \
            + ca.mtimes(np.full((1, num_slack),lam), var['eps', Nt])
        if lam_state is not None:
            obj += ca.mtimes(np.full((1,num_state_slack),lam_state), var['eps_state', Nt])
        ...
        con_tot = con_eq + con_ineq
        con = ca.vertcat(*con_tot)
...
#                B = self.__Bf @49 B_f + self.__Bd @ B_gp
        ...
        for t in range(self.__Nsim):
            ...
            self.__covariance[t + 1] = ca.mtimes(L, L.T)
    ...
    def __cost_saturation_lf(self, x, x_ref, covar_x, P):
        ...
        Z_x = ca.SX.eye(Nx) + 2 * ca.mtimes(covar_x_s, P_s)
        cost_x = ca.Function('cost_x', [x_s, P_s, covar_x_s],
                           [1 - ca.exp(-ca.mtimes(ca.mtimes((x_s.T, ca.solve(Z_x.T, P_s.T).T), x_s)))
                                   / ca.sqrt(ca.det(Z_x))])
    ...
    def __cost_saturation_l(self, x, x_ref, covar_x, u, covar_u, delta_u, Q, R, S):
        ...
        Z_x = ca.SX.eye(Nx) + 2 * ca.mtimes(covar_x_s, Q_s)
        Z_u = ca.SX.eye(Nu) + 2 * ca.mtimes(covar_u_s, R_s)

        cost_x = ca.Function('cost_x', [x_s, Q_s, covar_x_s],
                           [1 - ca.exp(-ca.mtimes(ca.mtimes(x_s.T, ca.solve(Z_x.T, Q_s.T).T), x_s))
                                   / ca.sqrt(ca.det(Z_x))])
        cost_u = ca.Function('cost_u', [u_s, R_s, covar_u_s],
                           [1 - ca.exp(-ca.mtimes(ca.mtimes(u_s.T, ca.solve(Z_u.T, R_s.T).T), u_s))
                                   / ca.sqrt(ca.det(Z_u))])
    ...
    def __constraint(self, mean, covar, H, quantile, ub, lb, eps):
        ...
        con_func = ca.Function('con', [mean_s, S_s, H_s, r],
                                [ca.mtimes(H_s, mean_s) + r * ca.mtimes(H_s, ca.diag(S_s))])
...
def lqr(A, B, Q, R):
    ...
    K = -np.array(scipy.linalg.solve(R + ca.mtimes(ca.mtimes(B.T, P), B), ca.mtimes(ca.mtimes(B.T, P), A)))
    eigenvalues, eigenvec = scipy.linalg.eig(A + ca.mtimes(B, K))
